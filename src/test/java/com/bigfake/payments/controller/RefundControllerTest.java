...
class RefundControllerTest {
    ...
    void getRefund_returns200() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/refunds/1"))
                ...
                .andExpect(jsonPath("$.refundId").value("RFD-TEST456"))
    ...
    void getRefundsForPayment_returns200() throws Exception {
        List<Refund> refunds = List.of(
                Refund.builder().id(1L).refundId("RFD-1").paymentId(1L).amount(new BigDecimal("20.00")).status(PaymentStatus.COMPLETED).build(),
                Refund.builder().id(2L).refundId("RFD-2").paymentId(1L).amount(new BigDecimal("30.00")).status(PaymentStatus.COMPLETED).build()
        );
