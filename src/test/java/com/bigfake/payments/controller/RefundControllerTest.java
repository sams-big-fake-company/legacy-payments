...
class RefundControllerTest {
    ...
    void processRefund_unauthenticated_returns401() throws Exception {
        RefundRequest request = RefundRequest.builder().paymentId(7L).amount(new BigDecimal("40.00")).build();
    ...
    @Test
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    void getRefund_unknownId_returns400FromPaymentException() throws Exception {
    ...
    void getRefundsForPayment_noRefunds_returnsEmptyList() throws Exception {
        when(refundService.getRefundsForPayment(8L)).thenReturn(List.of());

        mockMvc.perform(get("/api/v1/refunds/payment/8"))
                .andExpect(status().isOk())
