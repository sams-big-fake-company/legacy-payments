...
class RefundControllerTest {
    ...
    void processRefund_returns201() throws Exception {
        RefundRequest request = RefundRequest.builder()
                ...
                .build();
        when(refundService.processRefund(any(RefundRequest.class))).thenReturn(refund(1L, "RFD-ABC", "40.00"));

        mockMvc.perform(post("/api/v1/refunds")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.refundId").value("RFD-ABC"))
    ...
    void getRefund_returns200() throws Exception {
        when(refundService.getRefundById(1L)).thenReturn(refund(1L, "RFD-XYZ", "12.50"));

        mockMvc.perform(get("/api/v1/refunds/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.refundId").value("RFD-XYZ"))
    ...
    void getRefundsForPayment_returnsList() throws Exception {
        when(refundService.getRefundsForPayment(7L))
                .thenReturn(Arrays.asList(refund(1L, "RFD-1", "10.00"), refund(2L, "RFD-2", "5.00")));
