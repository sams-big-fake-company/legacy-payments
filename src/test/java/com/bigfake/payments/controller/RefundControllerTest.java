...
class RefundControllerTest {
    ...
    @Test
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    void getRefund_notFound_returns400() throws Exception {
        when(refundService.getRefundById(999L))
                .thenThrow(new PaymentException("Refund not found", "REFUND_NOT_FOUND"));
    ...
    void getRefundsForPayment_returns200() throws Exception {
        List<Refund> refunds = List.of(
                Refund.builder().id(1L).refundId("RFD-001").paymentId(1L)
                        .amount(new BigDecimal("20.00")).status(PaymentStatus.COMPLETED).build(),
                Refund.builder().id(2L).refundId("RFD-002").paymentId(1L)
                        .amount(new BigDecimal("30.00")).status(PaymentStatus.COMPLETED).build()
        );
        ...
        mockMvc.perform(get("/api/v1/refunds/payment/1"))
                ...
                .andExpect(jsonPath("$[0].refundId").value("RFD-001"))
                .andExpect(jsonPath("$[1].refundId").value("RFD-002"));
    }

    @Test
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    void getRefundsForPayment_empty_returns200() throws Exception {
