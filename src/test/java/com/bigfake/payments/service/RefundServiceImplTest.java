...
class RefundServiceImplTest {
    ...
    void setUp() {
        completedPayment = Payment.builder()
                .id(1L)
                .transactionId("TXN-REFUNDTEST001")
                .merchantId(1L)
    ...
    @Test
    void processRefund_notificationFailureDoesNotFailRefund() {
        when(paymentRepository.findById(1L)).thenReturn(Optional.of(completedPayment));
        when(refundRepository.findByPaymentId(1L)).thenReturn(Collections.emptyList());
        when(refundRepository.save(any(Refund.class))).thenAnswer(invocation -> {
            Refund r = invocation.getArgument(0);
    ...
    @Test
    void processRefund_existingRefundsWithNonCompletedStatus() {
    ...
    void getRefundsForPayment_success() {
        Refund refund1 = Refund.builder().id(1L).paymentId(1L).amount(new BigDecimal("10.00")).build();
        Refund refund2 = Refund.builder().id(2L).paymentId(1L).amount(new BigDecimal("20.00")).build();
    ...
    @Test
    void getRefundsForPayment_empty() {
        when(refundRepository.findByPaymentId(1L)).thenReturn(Collections.emptyList());
